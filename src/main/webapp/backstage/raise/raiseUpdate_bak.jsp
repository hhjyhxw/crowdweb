   		...
   		$.ajaxFileUpload({
               ...
               success:function(data){
				   if(data=="1"){
				    	...
				    	$("#_picture").val(data);
				    	$("#imgDiv").html("<img src=\"${pageContext.request.contextPath}/"+data+"\" width=\"300\" />"); 
...
<body>
<form action="${pageContext.request.contextPath}/hs/editTHsAdvertisement.action" method="post" name="form1" onsubmit="return onSubmitAction();">
<input name="id" type="hidden" class="_input" id="_id" size="36" value="${m.id }"/>
				...
				<td>
					<table border="0" style="width:100%" align="center" class="table5">
						<tr>
							...
							<td align="left" class="table5" colspan="2">&nbsp;<input name="title" class="input" id="_title"  style="width:50%" value="${m.title }" /></td>
						</tr>
						<tr>	
							<td align="right" class="table5 bgcolor5" width="15%">描述：&nbsp;</td>
							<td align="left" class="table5" colspan="2">&nbsp;<textarea name=descriptor id="_descriptor" rows="6" style="width:50%">${m.descriptor }</textarea></td>
						</tr>
						
						<tr>	
							<td align="right" class="table5 bgcolor5" width="15%">访问url：&nbsp;</td>
							<td align="left" class="table5" colspan="2">&nbsp;<input name="vistUrl" id="_vistUrl"  style="width:50%"  value="${m.vistUrl }" /></td>
						</tr>
						<tr>
							<td align="right" class="table5 bgcolor5" width="15%">状态：&nbsp;</td>
							<td align="left" class="table5" colspan="2">
							<select name="state"  id="_state"  style="width:50.3%;margin-left:4px;">
						...
						<tr>	
							...
							<td align="left" class="table5" colspan="2">&nbsp;<input name="validDate" id="_validDate"  style="width:50%"  value="<fmt:formatDate value="${m.validDate }" pattern="yyyy-MM-dd HH:mm:ss"/>" onfocus="WdatePicker({skin:'whyGreen',startDate:'%y-%M-%d 23:59:59',dateFmt:'yyyy-MM-dd HH:mm:ss'})" readonly="readonly"/></td>
						</tr>
						<tr>
							<td align="right" class="table5 bgcolor5" width="15%">失效日期：&nbsp;</td>
							<td align="left" class="table5" colspan="2">&nbsp;<input name="expiredDate" class="input" id="_expiredDate"  style="width:50%" value="<fmt:formatDate value="${m.expiredDate }" pattern="yyyy-MM-dd HH:mm:ss"/>" onfocus="WdatePicker({skin:'whyGreen',startDate:'%y-%M-%d 23:59:59',dateFmt:'yyyy-MM-dd HH:mm:ss'})" readonly="readonly"/></td>
						</tr>
						<tr>
							...
							<input type="file" name="file" id="_file" size="30" onchange="ajaxFileUpload(this)"/>
								<input type="hidden" name="picture" id="_picture" value="${m.picture }"/></td>
							<td>
								<div style="width:396px; height:90px; overflow:hidden; border:1px solid red;">      
								<div id="imgDiv"><img src="${pageContext.request.contextPath}/${m.picture }" style="width:396px;"/></div>   
								</div> 建议：预览图像素小于1980px × 450px。											
							</td>
						</tr>
					</table>
				</td>
