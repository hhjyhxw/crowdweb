...
JKB.loader = {
    ...
    switchMonitorCond: function(monitorCond)
    {
        ...
        if( monitorCond == 1 )
        {
            if( JKB.cache.th_dev_count != undefined && JKB.cache.th_dev_count > 0 )
                th_dev_sel_html = '<input style="width:56px;" type="text" name="th_monitor_count" value="'+JKB.cache.th_dev_count+'" /> 个';
            else
                th_dev_sel_html = '<input style="width:56px;" type="text" name="th_monitor_count" value="" /> 个';
        }
            ...
            for (var devid in cond_info['dev'])
            {
                ...
                if( JKB.cache.th_dev_name != undefined ) 
                {
                    ...
                    if( jQuery.inArray(devid, th_dev_name) != -1 )
                        th_dev_sel_html += '<input type="checkbox" name="th_dev_sel[]" value="' + devid + '" checked/> '+ cond_info['dev'][devid];
                    else
                        th_dev_sel_html += '<input type="checkbox" name="th_dev_sel[]" value="' + devid + '" /> '+ cond_info['dev'][devid];
                }
                else
                    th_dev_sel_html += '<input type="checkbox" name="th_dev_sel[]" value="' + devid + '" /> '+ cond_info['dev'][devid];
                th_dev_sel_html += '</div>';
    ...
    showDeleteTaskMessage: function()
    {
        ...
        if(jQuery('#delete_task_include').attr('checked') == false){
            include_status = 'no_include';
        }
        JKB.loader.setCookie(key, include_status);
	...
	getCheckedTasks: function(obj)
	{
		...
		if(!obj) obj = document.__form.task_comp;
		if (!obj)
		{
			alert(err);
			return '';
	...
	{   
		var task_ids = JKB.loader.getCheckedTasks();
		var ids = new Array; ids = task_ids.split(',');
	...
	loadZtree: function(param)
	{
	  ...
	  jQuery('#__load_ztree').load(getRandomUrl(ajax_url),function(){
	...
	loadServerListStatus: function(period, scale,page)
	{
		...
		if(view_type == 'list'){
			...
			if(status_type == 'list' && node_type == 'h'){
				var ajax_url = '/ajax_host_list.php?node_type=' + node_type + '&n_id=' + n_id + '&ent_node_id=' + ent_node_id;
			}else {
			...
			var ajax_url = '/ajax_server_list_status.php?type=' + status_type + '&ent_node_id=' + ent_node_id;
		}
